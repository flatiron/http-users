...
httpUsers.attach = function (options) {
  ...
  if (options && options.sshServer) {
    app.config.set('ssh:server:host', options.sshServer);
  }

  if (options && options.sshServerPort) {
    app.config.set('ssh:server:port', options.sshServerPort);
  }

  if (options && options.sshServerToken) {
    app.config.set('ssh:server:token', options.sshServerToken);
  }
};
