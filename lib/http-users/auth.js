...
UserStrategy.prototype.authenticate = function (req, callback) {
  ...
  this.app.resources.User.get(auth.username, function _auth(err, user) {
    attempts++;
    if (err) {
      if (err.error === 'not_found') {
        return forbidden(auth.username + ' not found', callback);
      }

      return attempts >= 3
        ? callback(err)
        : self.app.resources.User.get(auth.username, _auth);
    }
    ...
    if (user.status === 'frozen' || (requireActivation && user.status !== 'active')) {
      //
