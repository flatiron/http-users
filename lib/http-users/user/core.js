...
exports.routes = function (app) {
  ...
  app.router.before('/users/:username', function (username) {
    ...
    if (self.req.method === 'PUT' && self.req.body && self.req.body.password) {
      // Ensure password salts get set as well if for some reason it evaluates to false
      self.req.user['password-salt'] = self.req.user['password-salt'] || common.randomString(16);
      self.req.body.password = hash.md5(
        self.req.body.password,
        self.req.user['password-salt']
      );
    }
