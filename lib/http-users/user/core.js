...
exports.routes = function (app) {
  ...
  app.unauthorized.path('/users', function () {
    ...
    //
    // Check: GET to `/users/:email/unique` will check to see
    // if a specified email is unqiue and not registered already
    //
    this.get('/:email/unique', function (email) {
      var res = this.res;
      ...
      app.resources.User.byEmail(email, function (err, list) {
        ...
        return res.json(200, { available: !(list && list.length) });
      });
    });
  });
