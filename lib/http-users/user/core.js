...
exports.resource = function (app) {
  ...
  var User = app.resources.User = app.define('User', function () {
    ...
    this.string('username').sanitize('lower').sanitize('replace', /.+/, function (v) {
      return decodeURIComponent(v);
    }).pattern(disallowed.username instanceof RegExp
      ? disallowed.username
      : new RegExp(disallowed.username || /^[^@]+$/)
    );
...
exports.routes = function (app) {
  ...
  app.router.before('/users/:username', function (username) {
    ...
    if (this.req.method === 'PUT' && this.req.body && this.req.body.password) {
      this.req.body.password = hash.md5(
        this.req.body.password,
        this.req.user['password-salt'] || common.randomString(16)
      );
    }
  ...
  //
  // Extend the default update route of an user. Check if the e-mail was
  // changed and if the status should be set to `pending`.
  //
  app.router.put('/users/:username', function (username, next) {
    var res = this.res
      , req = this.req
      , data = req.body || {};

    app.resources.User.get(username, function (err, current) {
      if (err) return res.json(err.status || 500, err);

      //
      // reset the inviteCode and force the status to pending.
      // Note: make sure to send an e-mail asking for confirmation.
      //
      ...
      //
      // Update the user and return the user resource with the inviteCode
      // as it can be useful afterwards for sending e-mails, etc.
      //
      app.resources.User.update(username, data, function (err, user) {
        if (err) return res.json(err.status || 500, err);
        return res.json(200, app.resources.User._restricted(user));
      });
    });
  });
};
