...
exports.resource = function (app) {
  ...
  var User = app.resources.User = app.define('User', function () {
    ...
    //
    this.before('destroy', function (obj, callback) {
      app.resources.Organization.byMember(obj.username, function (err, orgs) {
        if (err) return callback(err);

        function each(org, next) {
          if (~org.owners.indexOf(obj.username) && org.owners.length === 1) {
            org.destroy(function (err) {
              if (err) return cb(err);
              next();
            });
          } else {
            org.removeMember(obj.username, function (err) {
              if (err) return cb(err);
              next();
            });
          }
  ...
  User.available = function () {
    return app.resources.Organization._available.apply
