...
exports.routes = function (app) {
  //
  // List Keys: GET to `/users/keys/` retuns list of all public keys
  //            for all users.
  //
  app.router.get('/users/keys', function () {
    var res = this.res;
  ...
  app.router.path('/users/:userid/keys', function () {
    ...
    this.get(function (userId) {
      var res = this.res;
      app.resources.User.keys(userId, function (err, keys) {
        return err
          ? res.json(500, err)
          : res.json(200, { keys: keys });
      });
    ...
    //
    // Create SSH Account: GET to `/users/:userid/keys/:keyid/sshEnable` create 
    //                     a ssh account in the server defined in the httpUsers config
    //
    this.get('/:keyid/sshEnable', function (username, keyname) {
      var res = this.res;
      app.resources.User.get(username, function (err, user) {
        ...
        user.createSshAccount(keyname, function (err, respond) {
          return err ? res.json(500, err) : res.json(200, respond);
        });
      });
    });
