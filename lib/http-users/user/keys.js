    ...
    map: function (doc) {
      if (doc.resource === 'User' && doc._attachments) {
        for (var key in doc._attachments) {
          emit(doc._id, { _id: doc._id, keyname: key });
        }
      }
    }
  });
...
exports.routes = function (app) {
  ...
  //
  app.router.before(/\/keys\/([\w\-\_\.]+)/, auth.basicAuth);
  
  //
  // Setup RESTful web service for `/keys`
  //
  app.router.path(/\/keys/, function () {
    //
    ...
    //
    // Get Key: GET to `/keys/:username/:keyname` returns the value
    //          of the CouchDB attachment.
    //
    this.get(/\/([\w\-\_\.]+)\/([\w\-\_\.]+)/, function (id, keyname) {
      var res = this.res;
    ...
    this.post(/\/([\w\-\_\.]+)/, addOrUpdateKey);
    this.post(/\/([\w\-\_\.]+)\/([\w\-\_\.]+)/, addOrUpdateKey);
  });
};